...
The model showed moderate predictive power, with an R-squared value of 0.39 and a root mean squared prediction error (RMSPE) of 2.5 years. 
While the model provides reasonable estimates, improvements could be made by exploring more complex ML approaches or additional predictive features.
...
$$age = `{r} intercept` - (`{r} diam_coef` \times diameter) + (`{r} height_coef` \times height) + (`{r} weight_coef` \times shell\_weight)$$

This equation means that for every 1 mm increase in height, the age of the abalone increases by `{r} round(height_coef, digits=2)` years (about `{r} round((height_coef*365))` days). 
